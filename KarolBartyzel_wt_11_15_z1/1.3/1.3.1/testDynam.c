...
void verifyingfunction(void * pointer);

int main(){
  void *handle=dlopen("libcb.so",RTLD_NOW);
  if(!handle)
    printf("\nERROR: Couldn't load library. Error:\n%s\n",dlerror());

  void (*createListBook)(struct ListBook*) = dlsym(handle,"createListBook");
  verifyingfunction(createListBook);
  void (*createTreeBook)(struct TreeBook*) = dlsym(handle,"createTreeBook");
  verifyingfunction(createTreeBook);
  void (*makeContact)(struct Contact*,char*,char*,char*,char*,char*,char*) = dlsym(handle,"makeContact");
  verifyingfunction(makeContact);

  void (*addContactToListBookC)(struct ListBook*,struct ListNode*) = dlsym(handle,"addContactToListBookC");
  verifyingfunction(addContactToListBookC);
  void (*addContactToTreeBookC)(struct TreeBook*,struct TreeNode*) = dlsym(handle,"addContactToTreeBookC");
  verifyingfunction(addContactToTreeBookC);

  void (*deleteContactFromListBook)(struct ListBook*,char*,char*) = dlsym(handle,"deleteContactFromListBook");
  verifyingfunction(deleteContactFromListBook);
  void (*deleteContactFromTreeBook)(struct TreeBook*,char*,char*) = dlsym(handle,"deleteContactFromTreeBook");
  verifyingfunction(deleteContactFromTreeBook);

  void (*sortListBookByNthElement)(struct ListBook*,int) = dlsym(handle,"sortListBookByNthElement");
  verifyingfunction(sortListBookByNthElement);

  void (*sortTreeBook)(struct TreeBook**,int) = dlsym(handle,"sortTreeBook");
  verifyingfunction(sortTreeBook);

  struct Contact* (*findContactInListBook)(struct ListBook*,char*,char*) = dlsym(handle,"findContactInListBook");
  verifyingfunction(findContactInListBook);
  struct Contact* (*findContactInTreeBook)(struct TreeBook*,char*,char*) = dlsym(handle,"findContactInTreeBook");
  verifyingfunction(findContactInTreeBook);

  void (*deleteListBook)(struct ListBook*) = dlsym(handle,"deleteListBook");
  verifyingfunction(deleteListBook);
  void (*deleteTreeBook)(struct TreeBook*) = dlsym(handle,"deleteTreeBook");
  verifyingfunction(deleteTreeBook);
...
void verifyingfunction(void * pointer){
    if(!pointer){
        printf("Error during loading symbol:\n%s", dlerror());
        exit(17);
    }
    return;
}
